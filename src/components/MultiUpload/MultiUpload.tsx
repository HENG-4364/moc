...
import { UploadStatus } from "./StatusUpload"
import { UploadSummary } from "./SummaryUpload"
...
// }
// `;
export function MultiUpload({ onFileUpload, defaultValue, setUploadedFiles, uploadedFiles }: FileUploadProps) {
      ...
      {(files.length > 0 || uploadedFiles?.length > 0) && (
        <>
          ...
          <div className="mt-4 flex gap-2">
            <Button variant="destructive" onClick={() => setFiles([])} disabled={uploading}>
            ...
            >
              <Upload className="mr-2" size={16} /> {uploading ? "រក្សាទុក..." : "រក្សាទុក"}
            </Button>
          </div>
        </>
      )}
