...
export type BusinessDirectoryCategoryDetailScreenProps = {
  dict: any;
  categoryDetail: any;
...
export default function BusinessDirectoryCategoryDetailScreen({
...
}: BusinessDirectoryCategoryDetailScreenProps) {
  ...
  const { data, loading } = useQuery(GET_PUBLIC_BUSINESS_DIRECTORY_LIST_QUERY, {
    variables: {
      ...
      filter: {
        businessCategoryId:
          Number(categoryDetail?.publicBusinessDirectoryCategoryDetail?.id) ||
          null,
  ...
  useEffect(() => {
    ...
    router.push(`/business-directory/category/${categoryDetail?.publicBusinessDirectoryCategoryDetail?.id}?${newSearchParams.toString()}`, { scroll: false });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (searchTerm.trim()) {
      router.push(
        `/business-directory/category/${(categoryDetail?.publicBusinessDirectoryCategoryDetail?.id)}?q=${encodeURIComponent(searchTerm)}`
      );
      setIsOpen(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-6 ">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>
                  {checkLang === "en-US"
                    ? categoryDetail?.publicBusinessDirectoryCategoryDetail
                      ?.name_en
                    : categoryDetail?.publicBusinessDirectoryCategoryDetail
                      ?.name_kh}
                  {data?.publicBusinessDirectoryList?.pagination?.total > 0 && (<span className="ms-1">( {data?.publicBusinessDirectoryList?.pagination?.total} )</span>)}
        ...
        <div className="py-4">
          <div className="text-2xl font-semibold">
            {checkLang === "en-US"
              ? categoryDetail?.publicBusinessDirectoryCategoryDetail?.name_en
              : categoryDetail?.publicBusinessDirectoryCategoryDetail?.name_kh}
          </div>
        </div>
          ...
          <div className="hidden lg:block w-full lg:w-[300px] py-4 border-r">
            <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-1 pr-3 space-y-2">
              ...
              {changeView === "list" && (
                <>
                  {(!data || loading) ? <>
                    ...
                    {data?.publicBusinessDirectoryList?.data?.length > 0 ? <><div className="grid grid-cols-1 md:grid-cols-2  gap-6">
                      {data?.publicBusinessDirectoryList?.data.map(
                        (business: any) => (
                          <CompanyCard
                            logoSrc={
